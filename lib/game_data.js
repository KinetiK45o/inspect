...
class GameData {
  constructor(update_interval, enable_update) {
    ...
    this.items_game_cdn_url =
      "https://raw.githubusercontent.com/SteamDatabase/GameTracking-CS2/2e4a5a7187866a3a70ef2ed6c9680b08223d8de7/game/csgo/pak01_dir/scripts/items/items_game_cdn.txt";
  ...
  addAdditionalItemProperties(iteminfo) {
    ...
    for (const sticker of iteminfo.stickers || []) {
      ...
      if (sticker.tintId) {
        name += ` (${
          this.csgo_english[`Attrib_SprayTintValue_${sticker.tintId}`]
        })`;
      }
    ...
    const rarityKey = Object.keys(this.items_game["rarities"]).find((key) => {
      return (
        parseInt(this.items_game["rarities"][key]["value"]) === iteminfo.rarity
      );
    });
    ...
    const qualityKey = Object.keys(this.items_game["qualities"]).find((key) => {
      return (
        parseInt(this.items_game["qualities"][key]["value"]) ===
        iteminfo.quality
      );
    });
